export function periodKeyOf(period, timeMs) {
    ...
    switch (period) {
        ...
        case "weekly": {
            ...
            const week = 1 +
                Math.round((t.getTime() - thursday.getTime()) / (7 * 24 * 3600 * 1000));
...
export function computeBudgetStatus(spentUsd, budgetUsd, config) {
    ...
    const state = spent >= hard ? "over" : spent >= warn ? "warn" : "ok";
...
export class BudgetGovernor {
    ...
    gate(status) {
        if (!status.blocked)
            return { allow: true };
        switch (status.hardAction) {
            case "block-new-requests":
                return {
                    allow: false,
                    reason: `budget exhausted: spent $${status.spentUsd.toFixed(2)} of $${status.budgetUsd.toFixed(2)}`,
                };
